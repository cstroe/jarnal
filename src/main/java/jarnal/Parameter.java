...
public class Parameter{
	...
	public Parameter(LinkedList list, int w){
		Jchar jc;
	...
	public boolean trapColor(Jchar test){
		...
		if(parent.page.parent.trapColors){
			int ii = test.getTrapColor();
			if(ii != -1){
				if(!parent.page.parent.trapc[ii]) return true;
			}
		}
		return false;
	}
	...
	public static int findWhole(String test, String targ, int n, boolean reverse, boolean wholeWord){
		...
		boolean found = Tools.foundWord(test, n, targ.length());
		while(!found){
			int start = n;
			if(!reverse) {
				start = n + 1;
			...
			found = Tools.foundWord(test, n, targ.length());
			if(found) break;
		}
		if(!found) nn = -1;
		else nn = n;
	...
	public static void setGraphics(Graphics2D gg2){
		if(Parameter.gg2 != null) Parameter.gg2.dispose();
		Parameter.gg2 = gg2;
	}
	...
	private String oldXML(int start, int len, LinkedList old){
		if(newStyle){
			...
			Parameter jp = new Parameter(old, width);
	...
	public JarnalSelection clipSel(){
		...
		Parameter jp = new Parameter(clip, width);
		return jp.clip();
	}
...
class Jspan{
	...
	public Jspan(Parameter parent, Jchar parms, String txt, int adv[], int dx, int dy){
		this.parent = parent;
		this.parms = parms;
		this.txt = txt;
		this.adv = adv;
	...
	public Jspan(Parameter parent, Jchar parms, int dx, int dy){
		this.parent = parent;
		this.parms = parms;
		adv=new int[1];
		adv[0] = dx; 
